...
Last lesson we set up Rust and you fought the borrow checker to something like a respectable draw. This is the lesson where that effort starts to pay off: by the end of it you'll have a program with a real address, live on devnet, that anyone with an internet connection can look up and call. Not a localhost demo you tear down after the screenshot — an actual deployed artifact. Tag along, terminal open, and we'll go end to end.
...
One prerequisite check. You need the Rust toolchain and Anchor installed, which you have from last lesson. What you do NOT need is a separate Solana CLI install: Anchor 1.0 bundles the whole Solana toolchain, which is a godsend if you ever lived through the old days of pinning Anchor and Solana releases against each other by hand and hoping the matrix held.
...
Anchor 1.0 changed the testing story: `anchor test` runs against Surfpool rather than the old `solana-test-validator`, and the default test template is LiteSVM, meaning Rust tests running in-process with no separate validator to spin up, warm up, and babysit while it eats a CPU core. Today that means the scaffolded test just runs, fast, and it should pass out of the box because you haven't changed anything yet. Run it, watch it go green, and then we're done with local for today.
...
Now, funding. Devnet SOL is free, that's what devnet is for, but it's rate-limited:
...
Next lesson we build the other half: client development with TypeScript and the wallet adapter, where that IDL file finally gets put to work and your program gets a front door humans can actually use. Happy deploying! 🚀
