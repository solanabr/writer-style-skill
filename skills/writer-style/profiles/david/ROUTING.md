...
|---|---|---|---|---|
| **Teach a Solana concept or mechanism in a course lesson** | **course-lesson** | 8–9.5 | `02-derivation-plain-mechanics` + `04` (+ `07` for length) | **helius ON** (evidence layer only). The pack's main working mode and an **extrapolated container** — no course content in the corpus. Facts-first is mandatory. Prose enumeration, not step lists. Code blocks are helius's register entirely; keep David's prose *around* them, never inside them. **Calibrated 2026-09-06 (`LESSONS.md`, R1): no announcement stubs or meta-narration; first person only as a receipt (≤8 "I" per 3k words); 8–11 sections of 250–450 words; cut restatement first. The marker appetite is LOW in this mode: seams and refrains did not read as a person, judgment did.** |
| **Open or close a course / motivate** | **course-opener** | 7.5–9 | `02-derivation-plain-mechanics` + `06-close-book-handoff` (**never `01`**) | **helius OFF** — nothing technical to evidence. Heat is tail-loaded by default; if a brief asks for a warm opener, that is off-corpus and belongs in the manifest. **R1 (`LESSONS.md`): the qualification opener (`01`) lost the blind A/B in this container; a course opener starts on the backward hook and the do-element. Same meta-narration ban and first-person budget as course-lesson.** |
